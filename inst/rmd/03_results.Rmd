...
```{r}
# Make dummy data for seeing effect of herd on irs
```
...
x <- x %>% gather(key, value, predicted_no_herd_no_irs:predicted_max_irs)
...
cols <- colorRampPalette(brewer.pal(n = 9,
                                    name = 'Spectral'))(length(unique(plot_data$group)))
